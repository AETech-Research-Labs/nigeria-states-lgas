...
nigeria_data = {
  ...
  "Katsina": [
    "Bakori", "Batagarawa", "Batsari", "Baure", "Bindawa", "Charanchi", "Dandume", "Danja", "Dan Musa",
    "Daura", "Dutsi", "Dutsin Ma", "Faskari", "Funtua", "Ingawa", "Jibia", "Kafur", "Kaita",
  ...
  "Ondo": [
    ...
    "Ese Odo", "Idanre", "Ifedore", "Ilaje", "Ile Oluji/Okeigbo", "Irele", "Odigbo", "Okitipupa",
  ...
  "Oyo": [
    ...
    "Ifedayo", "Ifelodun", "Irepo", "Iseyin", "Itesiwaju", "Iwajowa", "Kajola", "Lagelu", "Ogbomosho North",
    "Ogbomosho South", "Ogo Oluwa", "Olorunsogo", "Oluyole", "Ona Ara", "Orelope", "Ori Ire", "Oyo East",
    "Oyo West", "Saki East", "Saki West", "Surulere"
  ],
  "Plateau": [
    "Bokkos", "Barkin Ladi", "Jos East", "Jos North", "Jos South", "Kanam", "Kanke", "Langtang North",
...
with open("data/nigeria_states.json", "w") as f:
    json.dump(states, f, indent=2, ensure_ascii=False)

with open("data/nigeria_states.csv", "w") as f:
    writer = csv.writer(f)
    writer.writerow(["state_id", "state_name"])
    for i, state in enumerate(states, start=1):
        writer.writerow([i, state])

lgas_list = [{"state": state, "lga": lga} for state, lgas in nigeria_data.items() for lga in lgas]
with open("data/nigeria_lgas.json", "w", encoding="utf-8") as f:
    json.dump(lgas_list, f, indent=2, ensure_ascii=False)

with open("data/nigeria_lgas.csv", "w", newline="", encoding="utf-8") as f:
    writer = csv.writer(f)
    writer.writerow(["state", "lga"])
    for item in lgas_list:
        writer.writerow([item["state"], item["lga"]])

with open("data/nigeria_states_and_lgas.json", "w", encoding="utf-8") as f:
    json.dump(nigeria_data, f, indent=2, ensure_ascii=False)

with open("data/nigeria_states_and_lgas.csv", "w", newline="", encoding="utf-8") as f:
    writer = csv.writer(f)
    writer.writerow(["state", "lga"])
...
with open("data/nigeria_states_and_lgas.sql", "w", encoding="utf-8") as f:
    for state, lgas in nigeria_data.items():
        for lga in lgas:
            f.write("CREATE TABLE states (id INT PRIMARY KEY, name VARCHAR(100));\n")
            f.write("CREATE TABLE lgas (id INT PRIMARY KEY, state_id INT, name VARCHAR(100),\n")
            f.write("    FOREIGN KEY (state_id) REFERENCES states(id));\n\n")
            for i, state in enumerate(states, start=1):
                f.write(f"INSERT INTO states (id, name) VALUES ({i}, '{state}');\n")
            lga_id = 1
            for i, state in enumerate(states, start=1):
                for lga in nigeria_data[state]:
                    f.write(f"INSERT INTO lgas (id, state_id, name) VALUES ({lga_id}, {i}, '{lga}');\n")
                    lga_id += 1